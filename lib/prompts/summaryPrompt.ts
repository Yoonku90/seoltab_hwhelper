...
- **카드뉴스 확인 문제 (cardQuizHints) - 매우 중요**:
  - 오늘 수업 핵심 정리의 각 주요 섹션마다 1개씩 생성 (최대 6개, 최소 3개)
  - 빈칸 채우기 형식으로 **핵심 개념만** 묻기
  - **문제 생성 원칙**:
    1. **문맥을 고려**: detailedContent의 각 섹션에서 가장 중요한 핵심 개념을 추출
    2. **정답과 오답의 차이 명확히**: 보기 2개는 **서로 다른 개념**이어야 함 (동의어/유사어 금지)
    3. **과목별 특성 반영**:
       - **수학**: 공식, 정의, 성질 중심 (예: "이차함수 vs 일차함수", "미분 vs 적분", "사인 vs 코사인")
       - **과학**: 원리, 법칙, 용어 중심 (예: "광합성 vs 호흡", "산성 vs 염기성", "전압 vs 전류")
       - **영어**: 문법, 품사, 시제 중심 (예: "비교급 vs 최상급", "현재완료 vs 과거", "주어 vs 목적어")
       - **국어**: 문학 용어, 문법 중심 (예: "화자 vs 작가", "직설법 vs 간접법", "주어 vs 서술어")
    4. **문제 문장 구성**: 
