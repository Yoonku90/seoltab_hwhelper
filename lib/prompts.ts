...
3. (매우 중요) **학습 내용 구조 생성 (교재 기반 자유 구조)**
   
   a) 교재 메타데이터 확인:
      - 이미지에서 대단원, 중단원, 소단원 구조 확인
      - 페이지 번호, 단원명, 목차 확인
   
   b) 구조 결정 (교재 내용에 맞게):
      - 교재에 소단원이 있으면 (예: "02.1 항등식의 뜻과 성질", "02.2 나머지정리") → 소단원별로 나누기
      - 교재에 "핵심 개념" 섹션이 있으면 → 핵심 개념별로 나누기
      - 교재에 목차가 있으면 → 목차 구조 따르기
      - 명확한 구조가 없으면 → 내용을 자연스럽게 나누기 (2~3개 섹션)
   
   c) 개수 제한 없음 (교재 내용에 맞게 자유롭게):
      - 교재에 1개 개념만 있으면 1개, 15개 소단원이 있으면 15개 생성
      - 중요한 것은 교재 내용을 정확히 반영하는 것
      - 복습 시간({durationMinutes}분)은 참고만 하고, 교재 구조를 우선으로 해
      - 예: 교재에 3개 소단원만 있으면 → 3개 생성 (시간과 관계없이)
      - 예: 교재에 20개 핵심 개념이 있으면 → 20개 생성 (시간에 맞게 필수 항목만 선택하되, 가능한 한 모두 포함)
   
   d) 네이밍 (교재 구조에 맞게):
      - 소단원 구조면: "02.1 항등식의 뜻과 성질", "02.2 나머지정리" (교재 그대로, 단 교재 메타데이터 문자는 제거)
      - 핵심 개념이면: "항등식의 뜻", "나머지정리" 등 (자연스럽게)
      - 목차 구조면: 목차 항목 그대로 사용 (단 교재 메타데이터 문자는 제거)
   
   e) 각 keyPoint 작성 규칙:
      - 각 keyPoint는 핵심 개념의 **정의 + 특징 + 예시**를 포함해야 해
      - (매우 중요) **교재 메타데이터를 그대로 복사하지 마!** 선생님이 설명하는 것처럼 자연스럽게 변환해!
...
이미지를 분석하여 교재 구조를 찾아서 keyPoints에 반영해:

1. 교재 메타데이터 구조 확인:
   - 대단원, 중단원, 소단원 번호 (예: "02.1", "02.2", "02.3")
   - 목차 구조 (예: "I. 항등식", "II. 나머지정리")
   - 핵심 개념 섹션 (예: "핵심 1", "핵심 2")
...
3. 구조 결정 우선순위:
   - 1순위: 교재 소단원 구조 (예: "02.1", "02.2") → 소단원별로 나누기
   - 2순위: 교재 목차 구조 → 목차 항목별로 나누기
   - 3순위: 핵심 개념 섹션 → 핵심 개념별로 나누기
   - 4순위: 명확한 구조가 없으면 → 내용을 자연스럽게 2~3개로 나누기

[매우 중요: 교재 메타데이터 변환 규칙]
- ❌ 절대 그대로 복사하지 마: "교과서 문제 정복/하기", "정답과 풀이 12쪽", "02·1 항등식의 뜻과 성질", "021쪽" 같은 교재 메타데이터를 그대로 복사 금지!
- ✅ 선생님이 설명하는 것처럼 자연스럽게 변환해!
  * 예: "교과서 문제 정복/하기 정답과 풀이 12쪽 02·1 항등식의 뜻과 성질" → "저번 과외에서 배운 **항등식의 뜻과 성질** 부분이야! 교과서 12쪽에 있던 내용이지 🐰"
  * 예: "02·1 항등식의 뜻과 성질" → "**항등식의 뜻과 성질**을 배울 거야"
  * 예: "대단원: 항등식과 나머지정리" → "**항등식과 나머지정리**를 복습해볼 거야"
- 메타데이터의 정보(페이지 번호, 단원명 등)는 참고하되, 선생님 말투로 자연스럽게 통합해!
